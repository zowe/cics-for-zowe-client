...
Here, we'll create an action that adheres to the `IResourceAction` interface, and register it using the API's `resources.resourceExtender.registerAction` method.
...
const myCustomResourceAction: IResourceAction = {
...
export async function activate(ctx) {
  ...
  const myResourceLoggingAction: IResourceAction = {
    ...
    visibleWhen: (resource: IResource, context: IResourceContext) => {
      return resource.status === "ENABLED";
    },

    action: (resource: IResource, context: IResourceContext) => {
...
export async function activate(ctx) {
  ...
  commands.registerCommand("MY.CICSTASK.COMMAND", (cicsTaskId: string, cicsTranId: string, cicsRegionName: string, val: number) => {
  ...
  const myResourceLoggingAction: IResourceAction = {
    ...
    action: async (resource: IResource, context: IResourceContext) => {
      ...
      await commands.executeCommand("MY.CICSTASK.COMMAND", taskID, tranID, regionName, val);
    },
  };
