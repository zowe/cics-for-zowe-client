...
export class CicsCmciRestClient extends RestClient {
  ...
  public static convertObjectToXML(obj: any): string {
    this.log.trace(
      "Converting the following object to XML via xml2js:\n%s",
  ...
  public static async getExpectParsedXml(
    ...
    const data = await CicsCmciRestClient.getExpectString(
      session,
      resource,
      reqHeaders,
    );
   ...
   * @throws {ImperativeError} verifyResponseCodes fails
   */
  public static async deleteExpectParsedXml(
    session: AbstractSession,
    resource: string,
    reqHeaders: any[] = [],
  ): Promise<ICMCIApiResponse> {
    const data = await CicsCmciRestClient.deleteExpectString(
      session,
      resource,
      reqHeaders,
    );
  ...
  public static async putExpectParsedXml(
    ...
    const data = await CicsCmciRestClient.putExpectString(
      session,
      resource,
      reqHeaders,
      payload,
  ...
  public static async postExpectParsedXml(
    ...
    const data = await CicsCmciRestClient.postExpectString(
      session,
      resource,
      reqHeaders,
      payload,
    );
    const apiResponse: ICMCIApiResponse =
      CicsCmciRestClient.parseStringSync(data);
    return CicsCmciRestClient.verifyResponseCodes(apiResponse);
  }
  ...
  private static verifyResponseCodes(
    ...
    if (
      ...
      apiResponse.response.resultsummary.api_response1 ===
        CicsCmciRestClient.CMCI_SUCCESS_RESPONSE_1 &&
      apiResponse.response.resultsummary.api_response2 ===
        CicsCmciRestClient.CMCI_SUCCESS_RESPONSE_2
    ) {
      ...
      throw new ImperativeError({
        msg:
          CicsCmciMessages.cmciRequestFailed.message +
          "\n" +
  ...
  protected processError(original: IImperativeError): IImperativeError {
    ...
    try {
      ...
      this.log.debug(
        "Encountered an error trying to parse causeErrors as XML  - causeErrors is likely not JSON format",
      );
    }
