...
describe("Install Program Handler", () => {
  ...
  beforeEach(() => {
    ...
    // Mock installProgram
    (installProgram as jest.Mock).mockResolvedValue(mockResponse);
  });
  ...
  describe("processWithSession", () => {
    it("should call installProgram with correct parameters", async () => {
      await handler.processWithSession(mockParams, mockSession);

      expect(installProgram).toHaveBeenCalledWith(mockSession, {
        name: "TESTPROG",
        csdGroup: "TESTGRP",
        regionName: "TESTRGN",
        cicsPlex: "TESTPLEX",
    ...
    it("should handle errors from installProgram", async () => {
