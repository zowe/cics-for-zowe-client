...
import { ICMCIApiResponse, enableLocalFile } from "@zowe/cics-for-zowe-sdk";
...
/**
 * Command handler for enabling CICS LocalFiles via CMCI
 * @export
...
export default class LocalFileHandler extends CicsBaseHandler {
  public async processWithSession(params: IHandlerParameters, session: AbstractSession): Promise<ICMCIApiResponse> {
    const status: ITaskWithStatus = {
      statusMessage: "Enabling LocalFile from CICS",
      percentComplete: 0,
      stageName: TaskStage.IN_PROGRESS,
    };
    params.response.progress.startBar({ task: status });

    const response = await enableLocalFile(session, {
