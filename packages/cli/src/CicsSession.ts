...
export class CicsSession {
  ...
  public static createBasicCicsSession(profile: IProfile): Session {
    this.log.debug(
      "Creating a CICS session from the profile named %s",
      profile.name,
    );
    return new Session({
      type: "basic",
   ...
   * @returns {Session} - A session for usage in the CMCI REST Client
   */
  public static createBasicCicsSessionFromArguments(
   ...
   * @returns {Session} - A session for usage in the CMCI REST Client
   */
  public static async createSessCfgFromArgs(
    args: ICommandArguments,
    doPrompting = true,
    handlerParams?: IHandlerParameters,
  ): Promise<Session> {
    ...
    const sessCfgWithCreds =
      await ConnectionPropsForSessCfg.addPropsOrPrompt<ISession>(
        sessCfg,
        args,
        { doPrompting, parms: handlerParams },
      );
