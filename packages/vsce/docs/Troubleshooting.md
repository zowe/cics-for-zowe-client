...
Check that the [source of the error](https://github.com/zowe/vscode-extension-for-cics#checking-the-source-of-an-error) is from the Zowe Explorer for IBM CICS Transaction Server extension before refering to the troubleshooting documentation.
...
This Zowe V2 error appears when a cics profile entry is missing from the `zowe.schema.json` file. Currently, the installation of the Zowe Explorer for IBM CICS Transaction Server extension does not automatically add this cics entry. This is being worked on.
