...
import { TreeView, WebviewPanel, commands, window } from "vscode";
  ...
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      for (const regionTree of allSelectedNodes) {
        ...
        const db2transactionResponse = await getResource(
          regionTree.parentSession.session,
          {
            name: "CICSSystemParameter",
            regionName: regionTree.label,
            cicsPlex: regionTree.parentPlex
              ? regionTree.parentPlex!.getPlexName()
              : undefined,
        ...
        webText += "<tbody>";
        for (const systemParameter of db2transactionResponse.response.records
          .cicssystemparameter) {
        ...
        webText += "</tbody>";
        const webviewHTML = getParametersHtml(
          regionTree.getRegionName(),
          webText,
        );
