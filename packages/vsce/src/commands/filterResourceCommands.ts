...
export function getFilterLibrariesCommand(
  tree: CICSTree,
  ...
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] instanceof CICSLibraryTree
      ) {
      ...
      const pattern = await getPatternFromFilter(
        "Library",
        persistentStorage.getLibrarySearchHistory(),
      );
      if (!pattern) {
        return;
      }
...
export function getFilterDatasetsCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSLibraryTreeItem
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "Dataset",
        persistentStorage.getDatasetSearchHistory(),
      );
      if (!pattern) {
        return;
      }
  ...
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSProgramTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "Program",
        persistentStorage.getProgramSearchHistory(),
      );
      if (!pattern) {
        return;
      }
...
export function getFilterDatasetProgramsCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSLibraryDatasets
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
...
export function getFilterLocalFilesCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSLocalFileTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "Local File",
        persistentStorage.getLocalFileSearchHistory(),
      );
      if (!pattern) {
        return;
      }
  ...
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSTaskTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "Transaction ID",
        persistentStorage.getTransactionSearchHistory(),
      );
      if (!pattern) {
        return;
      }
...
export function getFilterTransactionCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSTransactionTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
...
export function getFilterTCPIPSCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSTCPIPServiceTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "TCPIP Services",
        persistentStorage.getTCPIPSSearchHistory(),
      );
      if (!pattern) {
        return;
      }
...
export function getFilterURIMapsCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSURIMapTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "URI Maps",
        persistentStorage.getURIMapSearchHistory(),
      );
      if (!pattern) {
        return;
      }
...
export function getFilterPipelinesCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSPipelineTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "Pipelines",
        persistentStorage.getPipelineSearchHistory(),
      );
      if (!pattern) {
        return;
      }
...
export function getFilterWebServicesCommand(
  tree: CICSTree,
  treeview: TreeView<any>,
) {
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] &&
        selection[selection.length - 1] instanceof CICSWebServiceTree
      ) {
        chosenNode = selection[selection.length - 1];
      } else {
      ...
      const pattern = await getPatternFromFilter(
        "Web Services",
        persistentStorage.getWebServiceSearchHistory(),
      );
      if (!pattern) {
        return;
      }
