 ...
 * @returns
 */
export function getFilterPlexResources(
  tree: CICSTree,
  ...
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      if (node) {
        ...
        selection[selection.length - 1] instanceof CICSRegionsContainer
      ) {
              ...
              for (const region of chosenNode.children) {
                if (region instanceof CICSRegionTree) {
                  if (region.getIsActive()) {
                    let treeToFilter;
                    if (resourceToFilter === "Programs") {
                      treeToFilter = region.children?.filter((child: any) =>
