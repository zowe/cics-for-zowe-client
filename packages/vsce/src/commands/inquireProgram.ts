 ...
 */
export function getInquireProgramCommand(
  tree: CICSTree,
  ...
  return commands.registerCommand(
    ...
    async (node) => {
      ...
      let resourceFolders;
      if (
        allSelectedNodes[0].getParent() instanceof CICSCombinedTransactionsTree
      ) {
        const cicsPlex: CICSPlexTree = allSelectedNodes[0]
          .getParent()
          .getParent();
        const regionsContainer = cicsPlex
          .getChildren()
          .filter((child) => child instanceof CICSRegionsContainer)[0];
        //@ts-ignore
        const regionTree: CICSRegionTree = regionsContainer
          .getChildren()!
          .filter(
            (region: CICSRegionTree) =>
              region.getRegionName() ===
              allSelectedNodes[0].parentRegion.getRegionName(),
          )[0];
      ...
      const programTree = resourceFolders.filter(
        (child) => child instanceof CICSProgramTree,
      )[0] as CICSProgramTree;
