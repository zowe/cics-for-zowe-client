          ...
          async (progress, token) => {
            ...
            for (const index in allSelectedNodes) {
              ...
              try {
                ...
                if (error.mMessage) {
                  // @ts-ignore
                  const [_resp, resp2, respAlt, eibfnAlt] =
                    splitCmciErrorMessage(error.mMessage);
                  ...
                  window.showErrorMessage(
                    `Something went wrong when performing a CLOSE - ${JSON.stringify(
                      error,
            ...
            for (const parentRegion of parentRegions) {
              try {
                const localFileTree = parentRegion.children.filter(
                  (child: any) =>
                    child.contextValue.includes("cicstreelocalfile."),
                )[0];
                ...
                if (
                  ...
                  if (
                    allLocalFileTreeTree.collapsibleState === 2 &&
                    allLocalFileTreeTree.getActiveFilter()
                  ) {
                ...
                window.showErrorMessage(
                  `Something went wrong when reloading local files - ${JSON.stringify(
                    error,
                    Object.getOwnPropertyNames(error),
                  ).replace(/(\\n\t|\\n|\\t)/gm, " ")}`,
                );
