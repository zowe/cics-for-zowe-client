        ...
        window.withProgress(
          ...
          async (progress, token) => {
            ...
            for (const parentRegion of parentRegions) {
              try {
                const taskTree = parentRegion.children.filter((child: any) =>
                  child.contextValue.includes("cicstreetask."),
                )[0];
                ...
                if (
                  ...
                  if (
                    allTaskTreeTree.collapsibleState === 2 &&
                    allTaskTreeTree.getActiveFilter()
                  ) {
                ...
                window.showErrorMessage(
                  `Something went wrong when reloading tasks - ${JSON.stringify(
                    error,
                    Object.getOwnPropertyNames(error),
                  ).replace(/(\\n\t|\\n|\\t)/gm, " ")}`,
                );
