...
import {
  findSelectedNodes,
  splitCmciErrorMessage,
} from "../../utils/commandUtils";
import constants from "../../utils/constants";
import { ICommandParams } from "../ICommandParams";

export function getDisableTransactionCommand(
  tree: CICSTree,
  ...
  return commands.registerCommand(
    ...
    async (clickedNode) => {
      const allSelectedNodes = findSelectedNodes(
        treeview,
        CICSTransactionTreeItem,
        clickedNode,
      );
        ...
        async (progress, token) => {
          ...
          for (const index in allSelectedNodes) {
            progress.report({
              ...
              increment:
                (parseInt(index) / allSelectedNodes.length) *
            ...
            try {
              await disableTransaction(
                currentNode.parentRegion.parentSession.session,
                {
                  ...
                  cicsPlex: currentNode.parentRegion.parentPlex
                    ? currentNode.parentRegion.parentPlex.getPlexName()
                    : undefined,
                },
              );
              ...
              if (error.mMessage) {
                // @ts-ignore
                const [_resp, resp2, respAlt, eibfnAlt] = splitCmciErrorMessage(
                  error.mMessage,
                );
                ...
                window.showErrorMessage(
                  `Something went wrong when performing a DISABLE - ${JSON.stringify(
                    error,
          ...
          for (const parentRegion of parentRegions) {
            try {
              const transactionTree = parentRegion.children.filter(
                (child: any) =>
                  child.contextValue.includes("cicstreetransaction."),
              )[0];
              ...
              if (
                parentRegion.parentPlex &&
                parentRegion.parentPlex.children.some(
                  (child) => child instanceof CICSRegionsContainer,
                )
              ) {
                const allTransactionTree =
                  parentRegion.parentPlex.children.filter((child: any) =>
                    child.contextValue.includes("cicscombinedtransactiontree."),
                  )[0] as CICSCombinedTransactionsTree;
                if (
                  allTransactionTree.collapsibleState === 2 &&
                  allTransactionTree.getActiveFilter()
                ) {
              ...
              window.showErrorMessage(
                `Something went wrong when reloading transactions - ${JSON.stringify(
                  error,
                  Object.getOwnPropertyNames(error),
                ).replace(/(\\n\t|\\n|\\t)/gm, " ")}`,
              );
