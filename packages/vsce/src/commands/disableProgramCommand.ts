...
export function getDisableProgramCommand(tree: CICSTree, treeview: TreeView<any>) {
  return commands.registerCommand("cics-extension-for-zowe.disableProgram", (clickedNode) => {
        ...
        for (const index in allSelectedNodes) {
          ...
          try {
            ...
            window.showErrorMessage(
              `Something went wrong when performing a new copy - ${JSON.stringify(error, Object.getOwnPropertyNames(error)).replace(
                /(\\n\t|\\n|\\t)/gm,
                " "
              )}`
            );
