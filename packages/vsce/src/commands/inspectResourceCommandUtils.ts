...
import { IResourceProfileInfo, SupportedResourceTypes } from "@zowe/cics-for-zowe-explorer-api";
...
import { getMetas, IContainedResource, IResource, IResourceMeta } from "../doc";
...
async function showInspectResource(
  ...
  resourceContext: IResourceProfileInfo,
...
export async function inspectResourceByName(context: ExtensionContext, resourceName: string, resourceType: string) {
  ...
  if (cicsRegion) {
    ...
    const resourceContext: IResourceProfileInfo = {
      profile: cicsRegion.profile,
...
async function loadResources(
  ...
  const resources = await resourceContainer.loadResources(SessionHandler.getInstance().getProfile(resourceContext.profile.name), resourceContext.regionName, resourceContext.cicsplexName);
