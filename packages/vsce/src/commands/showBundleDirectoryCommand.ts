...
export function showBundleDirectory() {
  return commands.registerCommand("cics-extension-for-zowe.showBundleDirectory", async (selectedBundle) => {
    if (!selectedBundle) {
      window.showErrorMessage(l10n.t(`No CICS bundle is selected`));
      return;
    }
    const bundleDir = selectedBundle.getContainedResource()?.resource.attributes.bundledir;
    if (!bundleDir) {
      window.showErrorMessage(l10n.t("Could not find bundle directory for {0}.", selectedBundle.getContainedResourceName()));
      return;
    }
    ...
    if (matchingZosProfile) {
      ...
      CICSLogger.debug(l10n.t("User picked z/OS profile: {0}", chosenProfileName));
    }
    try {
      // Get the profile object from the name
      const chosenProfile = zosProfiles.find((profile) => profile.name === chosenProfileName);
      if (!chosenProfile) {
        window.showErrorMessage(l10n.t("Could not find profile {0}", chosenProfileName));
        return;
      }
      ...
      try {
        ...
        CICSLogger.info(l10n.t("Executing command: zowe.uss.fullPath for {0}", bundleDir));
        ...
        if (message.includes(`Cannot resolve tree item for element 0/0:${chosenProfileName} from extension Zowe.vscode-extension-for-zowe`)) {
          CICSLogger.debug(l10n.t("Tree item resolution issue (expected): {0}", String(error)));
        } else {
          CICSLogger.error(l10n.t("Failed to load USS session: {0}", String(error)));
        }
      }
      await commands.executeCommand("zowe.uss.filterBy", ussNode);
    } catch (error) {
      window.showErrorMessage(l10n.t(`Unable to open bundle directory in USS view.`));
      CICSLogger.error(l10n.t("Failed to open bundle directory in USS view: {0}.", String(error)));
    }
  });
}
