      ...
      await window.withProgress(
        ...
        async (progress, token) => {
          ...
          for (const index in allSelectedNodes) {
            ...
            try {
              ...
              if (error.mMessage) {
                ...
                window.showErrorMessage(
                  `Something went wrong when performing a PHASEIN - ${JSON.stringify(
                    error,
          ...
          for (const parentRegion of parentRegions) {
            try {
              const programTree = parentRegion.children.filter((child: any) =>
                child.contextValue.includes("cicstreeprogram."),
              )[0];
              ...
              if (
                ...
                const allProgramsTree = parentRegion.parentPlex.children.filter(
                  (child: any) =>
                    child.contextValue.includes("cicscombinedprogramtree."),
                )[0] as CICSCombinedProgramTree;
                if (
                  allProgramsTree.collapsibleState === 2 &&
                  allProgramsTree.getActiveFilter()
                ) {
              ...
              window.showErrorMessage(
                `Something went wrong when reloading programs - ${JSON.stringify(
                  error,
                  Object.getOwnPropertyNames(error),
                ).replace(/(\\n\t|\\n|\\t)/gm, " ")}`,
              );
