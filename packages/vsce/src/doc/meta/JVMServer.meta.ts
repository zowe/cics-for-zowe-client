...
export const JVMServerMeta: IResourceMeta<IJVMServer> = {
  ...
  //   const status = jvmServer.attributes.jvmServerStatus;
  //   return `${CicsCmciConstants.CICS_JVMSERVER_RESOURCE}.${jvmServer.attributes.jvmServerStatus.trim().toUpperCase()}.${jvmServer.attributes.jvmServer}`;
  // },
getContext: function (jvmServer: Resource<IJVMServer>): string {
  ...
  //   const status = jvmServer.attributes.jvmServerStatus;
  //   if (jvmServer.attributes.jvmServerStatus.trim().toUpperCase() === "INACTIVE") {
  //     iconName += `-inactive`;
  //   }
  //   return iconName;
  // },
