    ...
    return (
      <>
        {filteredHiddenRows.length > 0 && (
          <tr className="sticky z-50 top-8 bg-(--vscode-panel-background) h-8 shadow-[0_0.5px_0_0_var(--vscode-disabledForeground)]">
        ...
        {showHiddenRows && filteredHiddenRows.map((row, idx: number) => (
          <tr key={`tr-hidden-${idx}`} className={`bg-(--vscode-panel-background) ${isDark ? "even:brightness-125" : "even:brightness-97"} h-8`}>
            {row.map((txt, idx) => (
