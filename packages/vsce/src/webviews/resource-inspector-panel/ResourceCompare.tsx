...
import { RegionResourceBreadcrumb } from "./Breadcrumb";
...
const ResourceCompare = ({ resources }: { resources: IResourceInspectorResource[]; }) => {
  ...
  useEffect(() => {
    ...
    const _headers = [
      ...
      ...resources.map(
        (res) => (
          <RegionResourceBreadcrumb
            profileName={res.context.profile.name}
            cicsplexName={res.context.cicsplexName}
            regionName={res.resource.eyu_cicsname}
            resourceName={res.name}
            menuData={res.actions.map(
              (ac) => {
                return {
                  label: ac.name,
                  value: ac.id,
                  resourceName: res.name,
                  resourceContext: res.context,
                  resources: [res]
                };
              }
            )}
