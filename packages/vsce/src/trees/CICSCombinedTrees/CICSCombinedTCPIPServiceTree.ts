...
export class CICSCombinedTCPIPServiceTree extends TreeItem {
  ...
  constructor(
    ...
    this.children = [
      new TextTreeItem(
        "Use the search button to display TCPIP Services",
        "applyfiltertext.",
      ),
    ];
    ...
    this.currentCount = 0;
    this.incrementCount =
      +`${workspace.getConfiguration().get("zowe.cics.allTCPIPS.recordCountIncrement")}`;
      ...
      async (_, token) => {
        ...
        try {
          ...
          window.showErrorMessage(
            `Something went wrong when fetching TCPIP Services TEST - ${JSON.stringify(
              error,
  ...
  public addTCPIPSUtil(
    newChildren: (CICSTCPIPServiceTreeItem | ViewMore)[],
    allTCPIPS: any,
    ...
    for (const tcpips of allTCPIPS) {
      ...
      const parentRegion = regionsContainer
        ...
        .filter(
          (child) =>
            ...
            child.getRegionName() === tcpips.eyu_cicsname,
        )?.[0] as CICSRegionTree;
      const tcpipsTree = new CICSTCPIPServiceTreeItem(
        tcpips,
        parentRegion,
        this,
      );
      tcpipsTree.setLabel(
        tcpipsTree.label
          ...
          .replace(
            tcpips.name,
            `${tcpips.name} (${tcpips.eyu_cicsname}) [Port #${tcpipsTree.tcpips.port}]`,
          ),
      );
          ...
          if (allTCPIPS) {
            ...
            this.addTCPIPSUtil(
              (this.getChildren()?.filter(
                (child) => child instanceof CICSTCPIPServiceTreeItem,
              ) ?? []) as CICSTCPIPServiceTreeItem[],
              allTCPIPS,
              count,
            );
            tree._onDidChangeTreeData.fire(undefined);
