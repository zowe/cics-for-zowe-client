...
export const executeAction = async (
  ...
  if (command === "action") {
    await handleActionCommand(message, instance, context);
  } else if (command === "refresh") {
    await handleRefreshCommand(message.resources, instance, context);
  }
};
...
const handleActionCommand = async (
  ...
  if (typeof action.action === "string") {
    await executeCommandAction(action.action, message.resources);
    if (action.refreshResourceInspector) {
      await handleRefreshCommand(message.resources, instance, context);
    }
  } else {
    const firstResource = message.resources[0];
    await action.action(
      firstResource.resource as ResourceTypeMap[keyof ResourceTypeMap],
      firstResource.context
    );
  }
};
  ...
  await window.withProgress(
    ...
    async (progress) => {
      ...
      try {
        ...
        const resourcesToDisplay = instance
          ? mergeWithExistingResources(instance.getResources(), updatedResources)
          : updatedResources;
...
const findMatchingResource = (
  ...
  return updatedResources.find((updated) =>
    resourcesMatch(existingResource, updated)
  );
};

const resourcesMatch = (
  existing: IResourceInspectorResource,
  updated: IResourceWithContext
): boolean => {
...
const showRefreshError = (error: unknown): void => {
  ...
  window.showErrorMessage(
    l10n.t("Something went wrong while performing Refresh - {0}", errorMessage)
  );
};
