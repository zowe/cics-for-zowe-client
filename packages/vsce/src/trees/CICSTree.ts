...
export class CICSTree implements TreeDataProvider<CICSSessionTree> {
  ...
  public async loadStoredProfileNames() {
    ...
    for (const profilename of persistentStorage.getLoadedCICSProfile()) {
      try {
        const profileToLoad =
          await ProfileManagement.getProfilesCache().loadNamedProfile(
            profilename,
            "cics",
          );
        // avoid accidental repeats
        if (
          !this.loadedProfiles.filter(
            (sessionTree) => sessionTree.label === profilename,
          ).length
        ) {
    ...
    try {
      ...
      if (configInstance.getTeamConfig().exists) {
        const currentProfile =
          await ProfileManagement.getProfilesCache().getProfileFromConfig(
      ...
      window.showErrorMessage(
        `Something went wrong while managing the profile - ${JSON.stringify(
          error,
  ...
  async addProfile() {
    try {
      ...
      const allCICSProfileNames: string[] = allCICSProfiles
        ? (allCICSProfiles.map((profile) => profile.profName) as unknown as [
            string,
          ])
        : [];
      // No cics profiles needed beforhand for team config method
      if (
        configInstance.getTeamConfig().exists ||
        allCICSProfileNames.length > 0
      ) {
        ...
        allCICSProfileNames
          ...
          .map((profileName) => {
            const osLocInfo = profileInfo.getOsLocInfo(
              profAllAttrs.find((p) => p.profName === profileName),
            );
            items.push(
              new FilterDescriptor(
                this.getProfileIcon(osLocInfo)[0] + " " + profileName,
              ),
            );
        ...
        const quickpick = Gui.createQuickPick();
        const addProfilePlaceholder = l10n.t(
          `Choose "Create new..." to define or select a profile to add to the CICS tree`,
        );
        ...
        try {
          ...
          if (configInstance.getTeamConfig().exists) {
            ...
            if (missingParamters.length) {
              ...
              if (
                missingParamters.includes(userPass[0]) ||
                missingParamters.includes(userPass[1])
              ) {
                const updatedProfile = await promptCredentials(
                  profile.name,
                  true,
                );
                if (!updatedProfile) {
                ...
                missingParamters = missingParamters.filter(
                  (param) => userPass.indexOf(param) === -1,
                );
              }
              if (missingParamters.length) {
          ...
          const plexInfo: InfoLoaded[] =
            await ProfileManagement.getPlexInfo(profile);
          // Initialise session tree
          newSessionTree = new CICSSessionTree(
            profile,
            getIconPathInResources("profile-dark.svg", "profile-light.svg"),
          );
          ...
          for (const item of plexInfo) {
            ...
            if (item.plexname === null) {
              ...
              if (item.group) {
                const newPlexTree = new CICSPlexTree(
                  item.plexname,
                  profile,
                  newSessionTree,
                  profile.profile.regionName,
                );
                newPlexTree.setLabel(
                  `${item.plexname} - ${profile.profile.regionName}`,
                );
                    ...
                    if (decision) {
                      if (decision === "Yes") {
                        const configInstance =
                          await ProfileManagement.getConfigInstance();
                        let updatedProfile;
                        if (configInstance.getTeamConfig().exists) {
                          const upd = {
                          ...
                          await configInstance.updateProperty({
                            ...upd,
              ...
              if (error.response !== "undefined" && error.response.status) {
                switch (error.response.status) {
                  case constants.HTTP_ERROR_UNAUTHORIZED:
                    window.showErrorMessage(
                      `Error: Request failed with status code 401 for Profile '${profile.name}'`,
                    );
                  ...
                  case constants.HTTP_ERROR_SERVER_ERROR:
                    window.showErrorMessage(
                      `Error: Request failed with status code 500 for Profile '${profile.name}'`,
                    );
                    break;
                  default:
                    window.showErrorMessage(
  ...
  async loadExistingProfile(label: string) {
    label = label.split(/ (.*)/)[1];
    const profileToLoad =
      await ProfileManagement.getProfilesCache().getLoadedProfConfig(label);
    const newSessionTree = new CICSSessionTree(profileToLoad);
    this.loadedProfiles.push(newSessionTree);
    const persistentStorage = new PersistentStorage("zowe.cics.persistent");
  ...
  async hideZoweConfigFile(allSelectedNodes: any[]) {
    for (const index in allSelectedNodes) {
      try {
        ...
        window.showErrorMessage(
          `Something went wrong when hiding the profile - ${JSON.stringify(
            error,
            Object.getOwnPropertyNames(error),
          ).replace(/(\\n\t|\\n|\\t)/gm, " ")}`,
        );
  ...
  async removeSession(
    ...
    this.loadedProfiles = this.loadedProfiles.filter(
      (p) => p.profile.name !== session.label?.toString(),
    );
  ...
  async updateSession(
    session: CICSSessionTree,
    configInstance: imperative.ProfileInfo,
  ) {
    await ProfileManagement.profilesCacheRefresh();
    const profileCache = await ProfileManagement.getProfilesCache();
    const profileToUpdate = profileCache.loadNamedProfile(
      session.label?.toString()!,
      "cics",
    );
    const currentProfile = await profileCache.getProfileFromConfig(
      profileToUpdate.name,
    );
    const teamConfigFilePath =
  ...
  private async getConfigLocationPrompt(action: string): Promise<string> {
    ...
    const location = await Gui.showQuickPick(
