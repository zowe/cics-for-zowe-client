...
export class CICSPipelineTree extends TreeItem {
  ...
  public async loadContents() {
    ...
    this.children = [];
    try {
      const pipelineResponse = await getResource(
      ...
      for (const pipeline of pipelinesArray) {
        const newPipelineItem = new CICSPipelineTreeItem(
          pipeline,
          this.parentRegion,
          this,
        );
        newPipelineItem.setLabel(
          newPipelineItem.label
            .toString()
      ...
      if (error.mMessage!.includes("exceeded a resource limit")) {
        window.showErrorMessage(
          `Resource Limit Exceeded - Set a Pipeline filter to narrow search`,
        );
