...
import { Gui, ZoweVsCodeExtension, MessageSeverity, imperative } from "@zowe/zowe-explorer-api";
...
export class CicsLogger {
    ...
    private static initializeImperativeLogger(logsPath: string): void {
        const zeLogLevel = CicsLogger.getLogSetting();
    ...
    public static getLogSetting(): string {
      this.zeLogLevel = vscode.workspace.getConfiguration("zowe").get("cics.logger");
    ...
    private static writeLogMessage(message: string, severity: MessageSeverity): void {
      ...
      if (CicsLogger.imperativeLogger == null) {
          return;
      }
