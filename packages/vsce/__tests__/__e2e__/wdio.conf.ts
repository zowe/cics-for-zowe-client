...
process.env.CICS_CLI_HOME = resolvePath("../ci");

if (process.env.CICS_TEST_DIR) {
  const homeDir = (process.env["CICS_CLI_HOME"] = joinPath(__dirname, relativePath(__dirname, process.env.CICS_TEST_DIR)));
  ...
  if (!existsSync(joinPath(homeDir, "zowe.config.json"))) {
    console.error(
      "CICS config was not present in CICS_TEST_DIR. Please run `CICS config init` in this directory, edit the config and re-run the script.",
    );
    process.exit(1);
  }
}
...
const prestDir = joinPath(__dirname, "..", "..");
console.log("================================== __dirname = " + __dirname + "==================================");
console.log("================================== prestDir = " + prestDir + "==================================");
  ...
  capabilities: [
    {
      ...
      browserVersion: process.env.CICS_TEST_VSCODE_VER || "stable",
      "wdio:vscodeOptions": {
