...
test('Resource Inspector on Program', async ({ page, request }) => {
  ...
  await expect(page).toHaveTitle(/workspace - code-server/);

  await page.locator('[aria-label="Zowe Explorer"]').first().click();

  await page.locator('div.pane-header[aria-label="Data Sets Section"]').click();
  await page.locator('div.pane-header[aria-label="Jobs Section"]').click();
  await page.locator('div.pane-header[aria-label="Unix System Services (USS) Section"]').click();

  await page.locator(`div.custom-view-tree-node-item-resourceLabel[aria-label='wiremock_localhost']`).click();
  await page.locator(`div.custom-view-tree-node-item-resourceLabel[aria-label='CICSEX61']`).click();
  await page.locator(`div.custom-view-tree-node-item-resourceLabel[aria-label='IYCWENK1']`).click();
  await page.locator(`div.custom-view-tree-node-item-resourceLabel[aria-label='Programs']`).click();
  await page.locator(`div.custom-view-tree-node-item-resourceLabel[aria-label='C128N']`).click({ button: "right" });
